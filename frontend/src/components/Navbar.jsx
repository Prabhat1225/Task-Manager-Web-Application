...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-orange-100 shadow-sm items-center'>
        <h2 className='cursor-pointer uppercase font-medium'>
          <Link to="/"> W3villa Technologies - Assignment - Task Manager Web Application </Link>
        </h2>
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              <li className="bg-blue-400 text-white hover:bg-blue-600 font-medium rounded-md">
        ...
        <div className={`absolute md:hidden right-0 top-0 bottom-0 transition ${(isNavbarOpen === true) ? 'translate-x-0' : 'translate-x-full'} bg-gray-100 shadow-md w-screen sm:w-9/12 h-screen`}>
          ...
          <ul className='flex flex-col gap-4 uppercase font-medium text-center'>
            {authState.isLoggedIn ? (
              <>
                <li className="bg-blue-400 text-white hover:bg-blue-600 font-medium transition py-2 px-3">
